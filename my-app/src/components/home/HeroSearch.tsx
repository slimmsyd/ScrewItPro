...
export default function HeroSearch({ onCta }: { onCta: () => void }) {
  ...
  const searchServices = useMemo(
    () => [
      { icon: "package-open" as const, label: t("search.ikea"), hint: t("search.mostBooked") },
      { icon: "armchair" as const, label: t("search.general"), hint: t("search.from49") },
      { icon: "bed-double" as const, label: t("search.bed"), hint: t("search.from69") },
      { icon: "monitor" as const, label: t("search.desk"), hint: t("search.from59") },
      { icon: "door-closed" as const, label: t("search.wardrobe"), hint: t("search.from99") },
      { icon: "briefcase" as const, label: t("search.office"), hint: t("search.forTeams") },
      { icon: "sofa" as const, label: t("search.large"), hint: t("search.from129") },
      { icon: "truck" as const, label: t("search.pickup"), hint: t("search.doorToDoor") },
      { icon: "sparkles" as const, label: t("search.whiteGlove"), hint: t("search.placedInspected") },
      { icon: "badge-check" as const, label: t("search.membership"), hint: t("search.from29mo") },
    ],
    [t]
  );
    ...
    >
      ...
      >
        ...
        >
          {mobile ? (
            ...
            <>
              {t("common.joinNow")} <ArrowRight size={17} color="var(--white)" />
            </>
          )}
        </button>
      </div>
      ...
      {open && (
        ...
        >
          ...
          >
            <span>{q.trim() ? t("search.matching") : t("search.popular")}</span>
            <span
              style={{
                display: "flex",
                alignItems: "center",
            ...
            >
              <Zap size={13} /> {t("search.instantQuote")}
            </span>
          </div>
          {filtered.length > 0 ? (
            <div
